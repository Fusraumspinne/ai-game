  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid gap-4 xl:grid-cols-[1.35fr_1fr]">
        ...
        <Panel><PanelHeader eyebrow="Übernahmeabwehr" title="Poison-Pill-Programm" description="Rechtsberatung und Bezugsrechte erschweren einen feindlichen Zugriff für 180 Tage." /><div className="mt-4 flex items-center justify-between gap-3 rounded-xl border border-amber-300/10 bg-amber-300/[0.035] p-3"><div><p className="text-xs text-amber-100">Einmalige Kosten</p><p className="mt-1 font-mono text-sm text-amber-300">{compactMoney(defenseCost)}</p></div><Icon name="shield" size={28} className="text-amber-300/50" /></div><ActionButton className="mt-4 w-full" variant="secondary" disabled={state.cash < defenseCost || state.takeoverDefenseDays > 0} onClick={() => dispatch({ type: "ACTIVATE_DEFENSE" })}>{state.takeoverDefenseDays > 0 ? `Abwehr aktiv · ${state.takeoverDefenseDays} Tage` : "Abwehr aktivieren"}</ActionButton></Panel>
      </div>
